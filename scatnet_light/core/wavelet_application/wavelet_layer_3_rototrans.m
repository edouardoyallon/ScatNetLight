...
function [U_Phi, U_Psi] = wavelet_layer_3_rototrans(U, filters_space, filters_rot, options_space, options_rot)
...
for p = 1:numel(U.signal)
    ...
    U_Phi_space.meta.resolution(p) = meta_Phi_space.resolution;
        ...
        U_Phi_space.meta.lambda_theta(:,p)=U.meta.lambda_theta(:,p);
        U_Phi_space.meta.resolution_theta(p)=U.meta.resolution_theta(p);
...
if (calculate_psi)
    ...
    for p_r=1:numel(Uorb_space.signal)
        ...
        if(~is_separable)
            ...
            % Indeed,
            % |x*psi_theta1|*psi_(theta2+pi)=conj(|x*psi_theta1|*psi_(theta2))
            %y=get_half_angles(y,4);
        %else
            % Indeed,
            % We are looking for |x*psi_theta|*psi_(theta+theta2))
        %   y=reshape_rototranslation_nonseparable(y);
        %end
        
        % Conjugate to get the negative frequency!
        y=get_negative_frequency(y,5);
...
% If we need to compute the averaging
if(output_avg)
    for p_phi_space=1:length(U_Phi_space.signal)
        y = U_Phi_space.signal{p_phi_space};
        
        if(is_separable)% reshape it (theta1+theta2,theta1-theta2)
            dtheta1=U_Phi_space.meta.resolution_theta(p_phi); % Sampling used in theta1
            y=reshape_rototranslation_separable(y,dtheta1); % Observe that this signal is now in full resolution in (theta1+theta2,theta1-theta2)!
        end
