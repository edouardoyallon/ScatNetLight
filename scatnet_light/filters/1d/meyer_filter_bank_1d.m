...
function filters = meyer_filter_bank_1d(sig_length,options)
	if nargin < 2
		options = struct();
	end
	
	parameter_fields = {'filter_type','Q','J', 'boundary'};
	...
	options = fill_struct(options, ...
		'filter_format', 'fourier_multires');
	options = fill_struct(options, ...
		'boundary', 'per');

	
	if ~strcmp(options.filter_type,'meyer_1d')
		error('Filter type must be ''meyer_1d''');
	end
	...
	filters.psi.filter = cell(1,options.J);
	filters.phi = [];
	...
	for j1 = 0:options.J-1
		temp = meyer_1d(N,2^j1);
		
		S = S+abs(temp).^2;
	end
	
	psi_ampl = sqrt(1/max(S));
	...
	% Calculate the associated low-pass filter
		...
		[~,filters.phi.filter] = meyer_1d(N,2^(options.J-1));
