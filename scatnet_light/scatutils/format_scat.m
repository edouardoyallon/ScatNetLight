...
% This function is let for legacy purpose, scat_to_tensor should be used
% instead
% Usages
...
function [out,meta] = format_scat(X,fmt)
	...
	for m = 0:M-1
		if isempty(X{1+m}.signal)
            ...
            sz=0;
        for j1 = 0:length(X{1+m}.signal)-1
				out{1+m}(sz+1:sz+numel(X{1+m}.signal{j1+1}))=(X{1+m}.signal{j1+1}(:));
               				sz=sz+numel(X{1+m}.signal{j1+1});
