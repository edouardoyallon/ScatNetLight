...
void exit_with_help()
{
	...
	"	5 -- triangular precomputed kernel (kernel values in training_set_file)\n"
	"	6 -- precomputed radial basis function kernel (kernel values in training_set_file)\n"
	"	7 -- triangular precomputed radial basis function kernel (kernel values in training_set_file)\n"
...
int read_problem_dense(const mxArray *label_vec, const mxArray *instance_mat)
{
	   ...
	   param.kernel_type == TRI_PRECOMPUTED_RBF)
	{
		...
		elements = mxGetNumberOfElements(instance_mat);
	...
	for(i = 0; i < prob.l; i++)
	{
		...
		if(param.kernel_type == TRI_PRECOMPUTED ||
		   param.kernel_type == TRI_PRECOMPUTED_RBF)
		{
			prob.x[i].dim = (int)(samples[tri_idx])+1;
			prob.x[i].values = (x_space+tri_idx);
			for(k = 0; k < prob.x[i].dim; k++)
				prob.x[i].values[k] = samples[tri_idx++];
		}
		else
		{
			prob.x[i].dim = sc;
			prob.x[i].values = (x_space+i*sc);
			for(k = 0; k < sc; k++)
	...
	else if(param.kernel_type == TRI_PRECOMPUTED ||
	        param.kernel_type == TRI_PRECOMPUTED_RBF)
		elements = mxGetNumberOfElements(instance_mat)+prob.l;
	else
	{
		for(i = 0; i < prob.l; i++)
		{
	...
	for(i = 0; i < prob.l; i++)
	{
		...
		if(param.kernel_type == TRI_PRECOMPUTED ||
		   param.kernel_type == TRI_PRECOMPUTED_RBF)
		{
			int dim = (int)(samples[tri_idx])+1;
			for(k = 0; k < dim; k++)
			{
				x_space[j].index = k+1;
				x_space[j].value = samples[tri_idx++];
				j++;
			}
		}
		else
...
int read_problem_sparse(const mxArray *label_vec, const mxArray *instance_mat)
{
	...
	double *samples;	// no need to worry: sparse is always double
	double *labels;
	...
	for(i=0;i<prob.l;i++)
	{	
		prob.x[i].dim = max_index;
		prob.x[i].values = (x_space+i*max_index);
		prob.y[i] = labels[i];
