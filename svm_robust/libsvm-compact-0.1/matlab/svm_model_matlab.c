...
#ifdef _DENSE_REP
	{
		num_of_feature = model->SV->dim;
		
		if(model->param.kernel_type == PRECOMPUTED ||
		   model->param.kernel_type == TRI_PRECOMPUTED ||
		   model->param.kernel_type == PRECOMPUTED_RBF ||
		   model->param.kernel_type == TRI_PRECOMPUTED_RBF)
			num_of_feature = 1;
		
#ifdef _FLOAT_REP
		rhs[out_id] = mxCreateNumericMatrix(model->l, num_of_feature, mxSINGLE_CLASS, mxREAL);
		sv_ptr = (float*) mxGetData(rhs[out_id]);
#else
		rhs[out_id] = mxCreateDoubleMatrix(model->l, num_of_feature, mxREAL);
		sv_ptr = mxGetPr(rhs[out_id]);
#endif
		
		for(i = 0; i < model->l; i++)
			for(j = 0; j < num_of_feature; j++)
				sv_ptr[j*(model->l)+i] = (model->SV+i)->values[j];
		out_id++;
	}
#else
	{
		...
		for(i = 0;i < model->l; i++)
		{
			   ...
			   model->param.kernel_type == TRI_PRECOMPUTED_RBF)
			{
				...
				ir[ir_index] = 0; 
				ptr[ir_index] = (double) model->SV[i][0].value;
				ir_index++;
				jc[i+1] = jc[i] + 1;
			}
			else
			{
				...
				while (model->SV[i][x_index].index != -1)
				{
					...
					ptr[ir_index] = (double) model->SV[i][x_index].value;
					ir_index++, x_index++;
				}
				jc[i+1] = jc[i] + x_index;
			}
...
struct svm_model *matlab_matrix_to_model(const mxArray *matlab_struct, const char **msg)
{
...
#ifdef _DENSE_REP
	{
...
#ifdef _FLOAT_REP
		sv_ptr = (float*) mxGetData(rhs[id]);
#else
		sv_ptr = mxGetPr(rhs[id]);
#endif
		...
		for(i=0;i<sr;i++)
		{
			...
			p->dim = sc;
#ifdef _FLOAT_REP
			p->values = (float *) malloc(sc * sizeof(float));
#else
			p->values = (double *) malloc(sc * sizeof(double));
#endif
			for(j=0;j<sc;j++)
			{
				p->values[j] = sv_ptr[j*sr+i];
			}
		}
...
#ifdef _FLOAT_REP
				model->SV[i][x_index].value = (float) ptr[j];
#else
