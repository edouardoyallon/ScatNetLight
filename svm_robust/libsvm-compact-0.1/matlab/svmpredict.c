...
void predict(mxArray *plhs[], const mxArray *prhs[], struct svm_model *model, const int predict_probability)
{
...
#ifdef _FLOAT_REP
	x.values = (float*)malloc(feature_number*sizeof(float));
#else
	x.values = (double*)malloc(feature_number*sizeof(double));
#endif
#else
	...
	for(instance_index=0;instance_index<testing_instance_number;instance_index++)
	{
		...
		if(mxIsSparse(prhs[1]) && 
		   ...
		   model->param.kernel_type != TRI_PRECOMPUTED &&
		   model->param.kernel_type != PRECOMPUTED_RBF &&
		   model->param.kernel_type != TRI_PRECOMPUTED_RBF) // prhs[1]^T is still sparse
