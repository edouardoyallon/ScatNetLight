...
void libsvmwrite(const char *filename, const mxArray *label_vec, const mxArray *instance_mat, const int precomputed_flag)
{
	...
	for(i=0;i<l;i++)
	{
		...
		for(k=low;k<high;k++)
			fprintf(fp," %ld:%g", ir[k]+!precomputed_flag, samples[k]);		
...
{	
	...
	if(nrhs == 3 || nrhs == 4)
	{
		...
		if(nrhs==4 && mxIsDouble(prhs[3]))
			precomputed_flag = mxGetScalar(prhs[3]);
		
		mxGetString(prhs[0], filename, mxGetN(prhs[0])+1);		

		if(mxIsSparse(prhs[2]))
			libsvmwrite(filename, prhs[1], prhs[2], precomputed_flag);
		else
		{
