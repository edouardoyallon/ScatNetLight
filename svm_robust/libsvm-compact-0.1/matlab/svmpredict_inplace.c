...
#ifndef _DENSE_REP
#error "svmpredict_inplace only available for dense representation"
#endif
...
void predict(mxArray *plhs[], const mxArray *prhs[], struct svm_model *model, const int predict_probability, const int mask_flag)
{
	...
	{
		testing_instance_number = (int)mxGetN(prhs[4]);
		ptr_mask = mxGetPr(prhs[4]);
	}
	label_vector_row_num = (int)mxGetM(prhs[0]);
	label_vector_col_num = (int)mxGetN(prhs[0]);

	if(label_vector_col_num!=(int)mxGetN(prhs[1]))
	{
		mexPrintf("Length of label vector does not match # of instances.\n");
		fake_answer(plhs);
		return;
	}
	if(label_vector_row_num!=1)
	{
		mexPrintf("label (1st argument) should be a vector (# of rows is 1).\n");
		fake_answer(plhs);
		return;
	}
	...
	for(instance_index=0;instance_index<testing_instance_number;instance_index++)
	{
		...
		if(!mask_flag)
		{
			...
			x.values = (ptr_instance+instance_index*feature_number);
		}
		else
		{
			target_label = ptr_label[(int)ptr_mask[instance_index]-1];
			x.values = (ptr_instance+((int)ptr_mask[instance_index]-1)*feature_number);
		}
...
{
	mexPrintf(
		"Usage: [predicted_label, accuracy, decision_values/prob_estimates] = svmpredict(testing_label_vector, testing_instance_matrix, model, 'libsvm_options', mask_vector)\n"
		"Parameters:\n"
		"  model: SVM model structure from svmtrain.\n"
		"  libsvm_options:\n"
		"    -b probability_estimates: whether to predict probability estimates, 0 or 1 (default 0); one-class SVM not supported yet\n"
		"  mask_vector: A sequence of indices indicating which vectors to test.\n"
		"Returns:\n"
		 ...
		 int nrhs, const mxArray *prhs[] )
{
	...
	{	
		mexPrintf("Instance matrix cannot be sparse\n");
		fake_answer(plhs);
		return;
	}
	...
	if(mxIsStruct(prhs[2]))
	{
		...
		predict(plhs, prhs, model, prob_estimate_flag, (nrhs==5));
		// destroy model
		svm_free_and_destroy_model(&model);
	}
	else
